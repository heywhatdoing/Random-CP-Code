...
int lazy[N * 4];
stack<pair<Mint, pair<ll, array<int, 4>>>> stk;
Mint f(Mint x) {
    return x * (x + 1) * half;
}
...
void update(ll d, int &b, int id1, int id2, int v = 1, int tl = 0, int tr = sz - 1) {
    if (b == 0 || rt[v] < d) return;
    int mid = (tl + tr) / 2;
    stk.push({sum[v], {cnt[v], {v, id1, id2, lazy[v]}}});
    if (tl == tr) {
        ll len = r[tl] - l[tl] + 1;
    ...
    if (lazy[v]) {
        stk.push({sum[v * 2], {cnt[v * 2], {v * 2, id1, id2, lazy[v * 2]}}});
        stk.push({sum[v * 2 + 1], {cnt[v * 2 + 1], {v * 2 + 1, id1, id2, lazy[v * 2 + 1]}}});
        sum[v * 2] = all[v * 2], cnt[v * 2] = 0, lazy[v * 2] = 1;
        sum[v * 2 + 1] = all[v * 2 + 1], cnt[v * 2 + 1] = 0, lazy[v * 2 + 1] = 1;
    }
    
    if (d <= lt[v]) {
        if (b >= cnt[v * 2]) {
            stk.push({sum[v * 2], {cnt[v * 2], {v * 2, id1, id2, lazy[v * 2]}}});
            b -= cnt[v * 2], lazy[v * 2] = 1;
...
void solve() {
    ...
    auto dnq = [&](auto dnq, int l, int r, vector<array<ll, 4>> ranges) -> void {
        ...
        while (!stk.empty()) {
            ...
            auto [y, data2] = data1;
            auto [v, id1, id2, la] = data2;
            // dbgm(x.x, y, v, id1, id2);
            if (l != id1 || r != id2) break;
            stk.pop();
            sum[v] = x, cnt[v] = y, lazy[v] = la;
        }
    };
